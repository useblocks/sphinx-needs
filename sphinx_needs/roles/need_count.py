    ...
    for node_need_count in found_nodes:
        ...
        if filter:
            ...
            if len(filters) == 1:
                ...
                amount = str(len(filter_needs(config, need_list, filters[0])))
            elif len(filters) == 2:
                need_list = prepare_need_list(all_needs)  # adds parts to need_list
                amount_1 = len(filter_needs(config, need_list, filters[0]))
                amount_2 = len(filter_needs(config, need_list, filters[1]))
