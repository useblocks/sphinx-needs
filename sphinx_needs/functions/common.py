...
def copy(
    ...
    if filter:
        result = filter_needs(NeedsSphinxConfig(app.config), needs.values(), filter, need)
        if result:
            need = result[0]
...
def check_linked_values(
    ...
    **Result 5: Two checks and a joint status**
    ...
    for link in links:
        ...
        if filter_string:
            try:
                if not filter_single_need(config, need, filter_string):
                    continue
...
def calc_sum(
    ...
    **Example 4**
    ...
    for check_need in check_needs:
        if filter:
            try:
                if not filter_single_need(config, check_need, filter):
                    continue
            except ValueError:
                pass
...
def links_from_content(
    ...
    if filter:
        ...
        for link in raw_links:
            if link not in filtered_links and filter_single_need(config, needs[link], filter):
