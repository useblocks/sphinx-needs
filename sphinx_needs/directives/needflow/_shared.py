...
The legend is the clearest case of that.  Each engine used to draw one inside the
picture, in its own syntax and to its own scope rule, so the same option produced two
different legends; the one built here is a document table instead, which means one
