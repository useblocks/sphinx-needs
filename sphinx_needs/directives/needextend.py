...
def extend_needs_data(data: SphinxNeedsData, config: NeedsSphinxConfig) -> None:
    """Use data gathered from needextend directives to modify fields of existing needs."""
    list_names = (
        ["tags", "links"]
        + [x["option"] for x in config.extra_links]
        + [f"{x['option']}_back" for x in config.extra_links]
    )  # back-links (incoming)
    link_names = [x["option"] for x in config.extra_links]
    ...
    for current_needextend in data.get_or_create_extends().values():
        ...
        # If it looks like a need id, but we haven't found one, raise an exception
        elif need_filter is not None and re.fullmatch(config.id_regex, need_filter):
            ...
            if current_needextend["strict"]:
                ...
                continue
        try:
            found_needs = filter_needs(config, all_needs.values(), need_filter)
...
def process_needextend(
    _app: Sphinx, _doctree: nodes.document, _fromdocname: str, found_nodes: List[nodes.Element]
) -> None:
    """Simply remove the needextend node from the doctree."""
    for node in found_nodes:
        removed_needextend_node(node)  # type: ignore
