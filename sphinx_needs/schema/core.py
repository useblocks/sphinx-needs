...
def recurse_validate_type_schmemas(
    ...
    if "local" in schema:
        ...
        warnings_local = get_ontology_warnings(
            ...
            need_path=need_path,
            user_message=user_message if is_root else None,
        )
    ...
    if "network" in schema:
        for link_type, link_schema in schema["network"].items():
            minmax_warnings: list[OntologyWarning] = []
            targets_ok: list[str] = []
            targets_nok: list[str] = []
            warnings_per_target: dict[str, list[OntologyWarning]] = {}
            for target_need_id in need[link_type]:  # type: ignore[literal-required]
                try:
                    ...
                    if is_root and user_message is not None:
                ...
                if link_schema.get("items"):
                    ...
                    if new_success:
                        targets_ok.append(target_need_id)
                    else:
                        targets_nok.append(target_need_id)
                    warnings_per_target[target_need_id] = new_warnings
                else:
                    targets_ok.append(target_need_id)
            nok_warnings = [
                warning
                for target_id in targets_nok
                for warning in warnings_per_target[target_id]
            ]
            minmax_success = True
            cnt_ok = len(targets_ok)
            cnt_nok = len(targets_nok)
            if "minItems" in link_schema:
                min_items = link_schema["minItems"]
                if cnt_ok < min_items:
                    rule = MessageRuleEnum.network_too_few_links
                    msg = f"Too few valid links of type '{link_type}' ({cnt_ok} < {min_items})"
                    if cnt_ok > 0:
                        msg += f" / ok: {', '.join(targets_ok)}"
                    if cnt_nok > 0:
                        msg += f" / nok: {', '.join(targets_nok)}"
                    minmax_warnings.append(
                        {
                            "rule": rule,
                            "severity": get_severity(rule, severity),
                            "validation_message": msg,
                            ...
                            "need_path": [*need_path, link_type],
                            "children": nok_warnings,  # user is interested in these
                        }
                    )
                    if is_root and user_message is not None:
                        minmax_warnings[-1]["user_message"] = user_message
                    minmax_success = False
            if "maxItems" in link_schema:
                max_items = link_schema["maxItems"]
                if cnt_ok > max_items:
                    rule = MessageRuleEnum.network_too_many_links
                    msg = f"Too many valid links of type '{link_type}' ({cnt_ok} > {max_items})"
                    if cnt_ok > 0:
                        msg += f" / ok: {', '.join(targets_ok)}"
                    if cnt_nok > 0:
                        msg += f" / nok: {', '.join(targets_nok)}"
                    minmax_warnings.append(
                        {
                            ...
                            "need_path": [*need_path, link_type],
                            # children not passed, no interest as user sees too many passed needs
                        }
                    )
                    if is_root and user_message is not None:
                        minmax_warnings[-1]["user_message"] = user_message
                    minmax_success = False
            if "unevaluatedItems" in link_schema and (
                minmax_success and not link_schema["unevaluatedItems"] and cnt_nok > 0
            ):
                rule = MessageRuleEnum.network_unevaluated_items
                msg = f"{cnt_nok} unevaluated links of type '{link_type}' found"
                if cnt_ok > 0:
                    msg += f" / ok: {', '.join(targets_ok)}"
                if cnt_nok > 0:
                    msg += f" / nok: {', '.join(targets_nok)}"
                minmax_warnings.append(
                    {
                        ...
                        "validation_message": msg,
                        "need": need,
                        "schema_path": [*schema_path, "validate", "network", link_type],
                        "need_path": [*need_path, link_type],
                        "children": nok_warnings,
                    }
                )
                if is_root and user_message is not None:
                    minmax_warnings[-1]["user_message"] = user_message
                minmax_success = False
            if not minmax_success:
                success = False
            filtered_minmax_warnings = filter_warnings_severity(
                config, minmax_warnings, severity
            )
            warnings.extend(filtered_minmax_warnings)
