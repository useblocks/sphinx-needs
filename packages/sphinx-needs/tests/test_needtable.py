...
def test_doc_build_html(test_app):
    ...
    build_warnings = warning.getvalue()
    # We set unique ID's for node.table, so the following exception shall not occur anymore.
    assert "WARNING: Any IDs not assigned for table node" not in build_warnings

    # The dead `style_col` option is accepted for compatibility but warns:
    # it has never had any effect (declared but never read).
    assert build_warnings.count("The 'style_col' option has never had any effect") == 1
    assert "test_styles.rst" in build_warnings
