...
- **Zero network.** A checkout renders. That is the whole point of committing it, and it is
  worth more than the ~30 MB: CI is 22 jobs per run, Read the Docs builds on every pull
  request, developers work offline, and a sandboxed agent session's network allowlist is set
