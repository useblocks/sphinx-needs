...
@pytest.mark.parametrize(
    "test_app", [{"buildername": "html", "srcdir": "doc_test/doc_need_id_from_title"}], indirect=True
)
def test_doc_build_html(test_app):
    ...
    assert len("US_A_STORY_TITLE_2FD447") == need_id_length + len(need_directive_story_prefix)
    assert len("US_CONTENT_ID_TEST_A313") == need_id_length + len(need_directive_story_prefix)
