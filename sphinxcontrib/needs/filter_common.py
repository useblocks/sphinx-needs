...
def process_filters(all_needs, current_needlist):
    ...
    if sort_key:
        try:
            ...
            logger.warning(
                "Sorting parameter {key} not valid: Error: {e}", key=sort_key, e=e
            )
    ...
    if not filter_code or filter_code.isspace():
        ...
        found_needs_by_string = filter_needs(
            all_needs_incl_parts, current_needlist["filter"]
        )
...
def filter_needs(needs, filter_string="", current_need=None):
    ...
    for filter_need in needs:
        try:
            ...
            logger.warning(
                "Filter {filter} not valid: Error: {e}", filter=filter_string, e=e
            )
...
def filter_single_need(need, filter_string="", needs=None, current_need=None) -> bool:
    ...
    try:
        ...
        raise NeedInvalidFilter(
            "Filter {0} not valid: Error: {1}".format(filter_string, e)
        )
    return result
