...
def setup(app):
    ...
    app.add_role(
        "need",
        XRefRole(nodeclass=NeedRef, innernodeclass=nodes.emphasis, warn_dangling=True),
    )
    ...
    app.add_role(
        "need_part",
        XRefRole(nodeclass=NeedPart, innernodeclass=nodes.inline, warn_dangling=True),
    )
    ...
    app.add_role(
        "need_count",
        XRefRole(nodeclass=NeedCount, innernodeclass=nodes.inline, warn_dangling=True),
    )
...
def prepare_env(app, env, _docname):
    ...
    if not hasattr(env, "needs_workflow"):
        ...
        for link_type in app.config.needs_extra_links:
            env.needs_workflow[
                "backlink_creation_{}".format(link_type["option"])
            ] = False
...
def check_configuration(_app: Sphinx, config: Config):
    ...
    for internal in INTERNALS:
        if internal in extra_options.keys():
            raise NeedsConfigException(
                'Extra option "{}" already used internally. '
                " Please use another name.".format(internal)
            )
        if internal in link_types:
            raise NeedsConfigException(
                'Link type name "{}" already used internally. '
                " Please use another name.".format(internal)
            )
    ...
    for link in link_types:
        if link in extra_options.keys():
            raise NeedsConfigException(
                "Same name for link type and extra option: {}."
