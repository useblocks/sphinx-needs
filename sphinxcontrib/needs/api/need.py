...
def add_need(
    ...
    **Usage**:
    ...
    node_need = sphinxcontrib.needs.directives.need.Need("", classes=style_classes)
...
def make_hashed_id(app, need_type, full_title, content, id_length=None):
    ...
    if type_prefix is None:
        raise NeedsInvalidException(
            "Given need_type {} is unknown. File {}".format(need_type, app.env.docname)
        )

    hashable_content = full_title or "\n".join(content)
    return "%s%s" % (
        type_prefix,
        hashlib.sha1(hashable_content.encode("UTF-8")).hexdigest().upper()[:id_length],
    )
