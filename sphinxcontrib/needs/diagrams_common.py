...
class DiagramBase(Directive):
    ...
    def create_target(self, target_name):
        ...
        targetid = "{targetname}-{docname}-{id}".format(
            targetname=target_name, docname=env.docname, id=id
        )
    ...
    def collect_diagram_attributes(self):
        ...
        if len(link_types) > 0:
            link_types = [
                link_type.strip() for link_type in re.split(";|,", link_types)
            ]
        ...
        if not scale.isdigit():
            raise Exception(
                'Needsequence scale value must be a number. "{}" found'.format(scale)
            )
        if int(scale) < 1 or int(scale) > 300:
            raise Exception(
                'Needsequence scale value must be between 1 and 300. "{}" found'.format(
                    scale
                )
            )
