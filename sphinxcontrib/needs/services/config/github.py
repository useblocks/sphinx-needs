...
GITHUB_LAYOUT = {
    ...
    "layout": {
        ...
        "head": [
            ...
            + ", ".join(
                [
                    '<<link("{value}", text="{value}", is_dynamic=True)>>'.format(
                        value=x
                    )
        ...
        "meta_left": [
            '<<meta("{value}", prefix="{value}: ")>>'.format(value=x)
            for x in EXTRA_DATA_OPTIONS
        ]
        + [
            '<<link("{value}", text="Link", prefix="{value}: ", is_dynamic=True)>>'.format(
                value=x
            )
            for x in EXTRA_LINK_OPTIONS
        ],
        "meta_right": [
            ...
            '<<meta_all(no_links=True, exclude=["layout","style",{}, {}])>>'.format(
                GITHUB_DATA_STR, CONFIG_DATA_STR
            ),
            "<<meta_links_all()>>",
        ],
        "footer_left": [
