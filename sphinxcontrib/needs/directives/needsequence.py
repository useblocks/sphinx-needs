    ...
    for node in doctree.traverse(Needsequence):
        ...
        if len(start_needs_id) == 0:
            raise NeedsequenceDirective(
                ...
                " File {}"
                ":{}".format(
                    {current_needsequence["docname"]}, current_needsequence["lineno"]
                )
            )
...
def get_message_needs(
    sender, link_types, all_needs_dict, tracked_receivers=None, filter=None
):
    msg_needs = []
            ...
            for rec_id in receiver_ids:
                if filter:
                    ...
                    if not filter_single_need(
                        all_needs_dict[rec_id], filter, needs=all_needs_dict.values()
                    ):
                        continue
