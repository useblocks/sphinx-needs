...
class NeedflowDirective(FilterBase):
    ...
    def run(self):
        ...
        if not scale.isdigit():
            raise Exception(
                'Needflow scale value must be a number. "{}" found'.format(scale)
            )
        if int(scale) < 1 or int(scale) > 300:
            raise Exception(
                'Needflow scale value must be between 1 and 300. "{}" found'.format(
                    scale
                )
            )
                ...
                for link in need_info[link_type["option"]]:
                    ...
                    if "." in link or "." in need_info["id_complete"]:
                        ...
                        if (
                            ...
                            or env.config.needs_flow_show_links
                        ):
