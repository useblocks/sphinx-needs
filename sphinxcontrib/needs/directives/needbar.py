...
def process_needbar(app, doctree, fromdocname):
    ...
    for node in doctree.traverse(Needbar):
        ...
        if not xlabels:
            ...
            if not current_needbar["ylabels"]:#we have a ylabels in the content:
        ...
        if not len(xlabels) == len(local_data[0]):
            raise Exception(f'{error_id} length of xlabels is not equal with sum of columns')
        
        #ensure length of ylabels == content rows
        if not len(ylabels) == len(local_data):
            raise Exception(f'{error_id} length of ylabels is not equal with sum of rows')
        ...
        for x in range(len(local_data_number)):
            if not current_needbar['horizontal']:
                bar = axes.bar(index[x], local_data_number[x], bottom = y_offset, label = ylabels[x], color=colors[x])
            else:
                bar = axes.barh(index[x], local_data_number[x], left = y_offset, label = ylabels[x], color=colors[x])
        ...
        if xlabels_rotation:
            ...
            if xlabels_rotation.isdigit():
                ...
                #see https://matplotlib.org/stable/gallery/images_contours_and_fields/image_annotated_heatmap.html?highlight=textcolor
        ...
        if rel_file_path not in env.images:
            figure.savefig(os.path.join(env.app.srcdir, rel_file_path), format="png")
