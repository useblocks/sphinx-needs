    ...
    for node in doctree.traverse(Needfilter):
        ...
        if current_needfilter["layout"] == "list":
            ...
            try:
                ...
                text = nodes.Text(
                    "PlantUML is not available!", "PlantUML is not available!"
                )
                para += text
                content.append(para)
        ...
        for need_info in found_needs:
            if current_needfilter["layout"] == "list":
                ...
                row += row_col_maker(
                    app, fromdocname, env.needs_all_needs, need_info, "title"
                )
                row += row_col_maker(
                    app, fromdocname, env.needs_all_needs, need_info, "type_name"
                )
                row += row_col_maker(
                    app, fromdocname, env.needs_all_needs, need_info, "status"
                )
                row += row_col_maker(
                    app,
                    fromdocname,
                ...
                row += row_col_maker(
                    app, fromdocname, env.needs_all_needs, need_info, "tags"
                )
                tbody += row
            elif current_needfilter["layout"] == "diagram":
                # Link calculation
