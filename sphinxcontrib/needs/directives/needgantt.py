...
class NeedganttDirective(FilterBase, DiagramBase):
    ...
    def run(self):
        ...
        starts_after_links = get_link_type_option(
            "starts_after_links", env, self, "links"
        )
        ...
        duration_option = self.options.get(
            "duration_option", env.app.config.needs_duration_option
        )
        completion_option = self.options.get(
            "completion_option", env.app.config.needs_completion_option
        )
